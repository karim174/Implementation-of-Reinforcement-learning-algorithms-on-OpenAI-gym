...
class QLearner(object):
  ...
  def __init__(
    ...
    q_tp1_tar = q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False)
    if double_q:
      self.max_a = tf.argmax(q, axis=1)
    else:
      self.max_a = tf.argmax(q_tp1_tar, axis=1)

    q_tp1_tar_maxa = tf.reduce_sum(tf.multiply(q_tp1_tar, tf.one_hot(self.max_a, self.num_actions)), axis=1)
    q_tar=self.rew_t_ph+gamma*((1-self.done_mask_ph)*q_tp1_tar_maxa)
    q_taken = tf.reduce_max(q*tf.one_hot(self.act_t_ph, self.num_actions), axis=1)
    self.total_error = huber_loss(q_taken-q_tar)
  ...
  def step_env(self):
    ...
    if self.model_initialized:
      ...
      act = np.random.randint(0, self.num_actions - 1)

    #self.env.render()
    last_obs, reward, done, _ = self.env.step(act)
  ...
  def update_model(self):
    ...
    if (self.t > self.learning_starts and \
      ...
      self.session.run(self.train_fn,
                       {self.obs_t_ph: obs_batch, self.act_t_ph: act_batch,
