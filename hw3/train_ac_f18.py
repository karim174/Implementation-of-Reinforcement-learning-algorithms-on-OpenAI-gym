...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        # estiamting the Q value
        v_s_tp1 = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no : next_ob_no})
        v_s_t = self.sess.run(self.critic_prediction, feed_dict= {self.sy_ob_no : ob_no})
        re_n_arr=np.array(re_n)
        terminal_n_arr=np.array(terminal_n)
        q_n = re_n_arr+self.gamma*v_s_tp1*(1-terminal_n_arr)
        #raise NotImplementedError
        adv_n = q_n-v_s_t
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            V_s_tp1 = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
            V_s = re_n_arr + self.gamma * V_s_tp1 * (1 - terminal_n_arr)
            for _ in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, feed_dict={self.sy_target_n:V_s,self.sy_ob_no: ob_no})
